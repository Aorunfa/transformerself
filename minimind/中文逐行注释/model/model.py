...
# 定义 RMSNorm 类，实现一种归一化方法，归一化的对象类似于 LayerNorm，但计算方式不同
...
def precompute_pos_cis(dim: int, end: int, theta: float = 10000.0):
    ...
    freqs = torch.outer(t, freqs).float()  # 计算外积, m * ro
    pos_cis = torch.polar(torch.ones_like(freqs), freqs)  # 计算复数形式的位置编码, 极坐标在转换为复数的笛卡尔坐标表示
    return pos_cis
...
def apply_rotary_emb(xq, xk, pos_cis):
    ...
    xq_out = torch.view_as_real(xq_ * pos_cis).flatten(3)  # 应用旋转位置编码, 旋转 由于pos模为1 不改变x的模值, 展平回原来的形状
...
class Attention(nn.Module):
    def __init__(self, args: LMConfig):
        ...
        self.flash = hasattr(torch.nn.functional, 'scaled_dot_product_attention') and args.flash_attn  # 判断是否使用 Flash Attention, 减少内存使用 提高计算效率
    ...
    def forward(self, x: torch.Tensor, pos_cis: torch.Tensor, use_kv_cache=False):
        ...
        if use_kv_cache and self.eval():  # 如果使用 KV 缓存且在评估模式下, 用于inference加速, 减少冗余计算
            ...
            xq, xk, xv = self.wq(x), self.wk(x), self.wv(x)  # 计算 Q, K, V, for training
        ...
        xq = xq.transpose(1, 2)  # 调整 Q 的形状, shape(bs, n_heads, max_seq_len, d_model // n_heads)
        ...
        if self.flash:
            output = torch.nn.functional.scaled_dot_product_attention(xq, xk, xv, attn_mask=None,
                                                                      ...
                                                                      is_causal=True)  # 使用 Flash Attention， is_causal=True 自动mask
        else:
...
class Transformer(PreTrainedModel):
    ...
    def __init__(self, params: LMConfig = None):
        ...
        if not params:
            params = LMConfig()
        self.params = params
        self.vocab_size = params.vocab_size # 词表长度
        ...
        self.tok_embeddings.weight = self.output.weight                     # 共享词嵌入和输出层的权重, 同步更新
        pos_cis = precompute_pos_cis(self.params.dim // self.params.n_heads, self.params.max_seq_len)  # 预计算位置编码
        self.register_buffer("pos_cis", pos_cis, persistent=False)  # 注册位置编码缓冲区, persistent False 模型导出不被保存
        ...
        self.OUT = CausalLMOutputWithPast()  # 初始化输出对象, 使用transformer的输出结构
    ...
    def forward(self, tokens: Optional[torch.Tensor] = None, targets: Optional[torch.Tensor] = None,
        ...
        if targets is not None:
            ...
            self.last_loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=-1)  # 计算交叉熵损失, 自动对预测值进行softmax，ignore_index忽略目标值对应位置的损失
        else:
    ...
    def generate(self, idx, eos, max_new_tokens, 
                 temperature=0.7, 
                 top_k=None, 
                 stream=True, 
                 ...
                 use_kv_cache=True):
        """
        temperature: 改变sofmax前logits的分布， 使其更尖锐或更加平滑
        top_k：只在top_k字符中进行sample, 减小长尾输出；通常搭配temperature > 0进行使用
        eos: 结束符号
        """
        ...
        while idx.shape[1] < max_new_tokens - 1:  # 当生成的 token 数量小于最大数量时
            ...
            if temperature == 0.0:  # 如果温度为 0，直接选择概率最高的 token
                _, idx_next = torch.topk(logits, k=1, dim=-1) # 输出更加连贯
            else:
                logits = logits / temperature  # 调整 logits
                """
                temperature调整logits分布, 
                temperature < 1: 相当于倍乘，logits的分布差异更大，分布概率更尖锐
                temperature > 1: 相当于倍除，logits的分布差异更小，分布概率更平滑
                """
                if top_k is not None:  # 如果设置了 top-k 采样
                    v, _ = torch.topk(logits, min(top_k, logits.size(-1)))
                    logits[logits < v[:, [-1]]] = -float('Inf')  # 将小于 top-k 的 logits 设为负无穷， 只在top-k里面进行sample，保证多样性的同时减少长尾词语的输出

                probs = F.softmax(logits, dim=-1)  # 计算概率
                idx_next = torch.multinomial(probs, num_samples=1, generator=None)  # 采样下一个 token， 答案输出更加多样性
...
if __name__ == '__main__':
    # Rope
    pos_cis = precompute_pos_cis(64, 50)
    # print(pos_cis.shape)
    # print(pos_cis)
    xq = torch.ones((1, 50, 8, 64))
    xk = torch.ones((1, 50, 8, 64))
    xq, xk = apply_rotary_emb(xq, xk, pos_cis)
    print(xk.shape)
    xk = repeat_kv(xk, 2)
    print(xk.shape)
    ...
    Rope - done
    Rmsnorm - doen
    decorder-only: for text gen task 
    """