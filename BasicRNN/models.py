...
class RNN1(nn.Module):
    """
    a(t) = g0(W0 * [x, a(t - 1)] + bias), encoder a(t)
    W0 shape(hidden_units, len(x) + len(a(t - 1)) )
    
    y(t) = g1(W1 * a(t) + bias), decoder a(t)
    W1 shape(len(x), len(a(t))) == nn.Linear(len(a(t)), len(x))
    """
    def __init__(self, hidden_units=32):
        super().__init__()
        self.hidden_units = hidden_units                            # 状态a(t)维度
        self.linear_a = nn.Linear(hidden_units + EMBEDDING_LENGTH,
                                  hidden_units)                     # W0 for a(t) cacularion
        self.linear_y = nn.Linear(hidden_units, EMBEDDING_LENGTH)   # W1 for y(t) cacularion
        self.tanh = nn.Tanh()

    def forward(self, word: torch.Tensor):
        # word shape: [batch, max_word_length, embedding_length]
        batch, max_word_length = word.shape[0:2]
        ...
        a = torch.zeros(batch, self.hidden_units, device=word.device) # 第一次初始化a
        x = torch.zeros(batch, EMBEDDING_LENGTH, device=word.device)  # 第一次输入初始化x
        
        for i in range(max_word_length):
            next_a = self.tanh(self.linear_a(torch.cat((a, x), 1)))   # 横向拼接[a, x]
            hat_y = self.linear_y(next_a)                             # shape(batch, embedding_length)
            output[i] = hat_y
            x = word[i]
            a = next_a
...
class RNN2(torch.nn.Module):
    ...
    def __init__(self, hidden_units=64, embeding_dim=64, dropout_rate=0.2):
        ...
        self.encoder = nn.Embedding(EMBEDDING_LENGTH, embeding_dim)             # 可学习参数矩阵 shape(EMBEDDING_LENGTH, embeding_dim)
        self.rnn = nn.GRU(embeding_dim, hidden_units, 1, batch_first=True)      # 表示输入的张量第一个维度是不是batch
        self.decoder = torch.nn.Linear(hidden_units, EMBEDDING_LENGTH)          # y(t) = decoder(a(t))
