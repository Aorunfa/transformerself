...
## 五. RNN补充
  ...
  RNN基本原理可以概括为，通过维护一个中间状态```a(t)```，捕捉数据时序依赖关系。t时刻中间状态```a(t)```由t-1刻状态、t时刻的输入通过可学习参数矩阵W进行转换```a(t) = W([a(t-1), x(t)])```，```[a(t-1), x(t)]```表示横向拼接。t时刻输出有解码器对隐状态```a(t)```进行解码```y(t) = decoder(a(t))```。  
