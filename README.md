...
线性层表示这些矩阵。  
* FFN前馈神经网络, 隐藏层维度设置`4*d_model`，特征在更高维的隐空间交互，实现类似特征增强的效果, 4这个值目前看没太多意义，应该是基于最佳实验原则设计。
...
* prtrained: 采用自回归语言模型训练方式，训练目标为预测下一个token

* finetune: 采用sft监督指令微调，对每一条input-out数据对处理为特殊问答模版input进行自回归训练
